...
async def login_submit(
    ...
    ziel = next if next.startswith("/") else "/"
    # HTML-Response statt Redirect damit Cookie sicher übertragen wird
    html = f"""<!DOCTYPE html>
<html><head>
<meta charset="UTF-8">
<meta http-equiv="refresh" content="0;url={ziel}">
<script>window.location.href="{ziel}";</script>
</head><body>Wird weitergeleitet...</body></html>"""
    from fastapi.responses import HTMLResponse as _HTMLResponse
    response = _HTMLResponse(content=html, status_code=200)
    response.set_cookie(
        COOKIE_NAME, token,
...
async def logout(request: Request):
    ...
    if user and user.username == "demo":
        try:
            ...
            pass
    response = redirect(request, "/login", 303)
